...
import { TOKENS_BASE_MAINNET, TOKENS_BASE_SEPOLIA, TOKENS_ARBITRUM_MAINNET, TOKENS_ARBITRUM_SEPOLIA, TOKENS_WARDEN_TESTNET } from '@/types/chat';
import { useNetwork } from '@/context/NetworkContext';
import { getNetworkTokens } from '@/types/chat';

// Type for supported token symbols
type TokenSymbol = keyof typeof TOKENS_BASE_MAINNET;
...
const TokenSendComponent: React.FC<TokenSendComponentProps> = ({ parsedData, onConfirm, onCancel, sessionId }) => {
  const { networkType } = useNetwork(); // Add this hook to get current network
  const tokens = getNetworkTokens(networkType); // Get tokens for current network
