...
function UserProfileButton() {
  ...
  const openExplorer = () => {
    if (user.wallet?.address) {
      const baseUrl = networkType === 'warden_testnet'
        ? 'https://warden-testnet.basescan.org/address/'
          ...
          <div className="p-4 border-b border-white/10">
            ...
            <div className="flex gap-2 mt-3">
              ...
              >
                ...
                View on {networkType === 'warden_testnet' ? 'Warden Testnet' : 'Warden'}
              </button>
            </div>
          </div>
...
const NavBar: React.FC<NavBarProps> = ({ 
  ...
  const NetworkSelector = () => {
    ...
    const isValidNetwork = networkType === 'warden_testnet';
    ...
    return (
      <div className="relative">
        ...
        >
          <span className="mr-4">
            {!isValidNetwork ? 'Incorrect Network' : 'Warden Testnet'}
          </span>
        ...
        {isOpen && (
          <div className="absolute z-50 mt-3 min-w-[160px] bg-[#112240] border border-white/20 rounded-lg shadow-lg">
            <button
              onClick={() => {
                setNetworkType('warden_testnet');
                setIsOpen(false);
              }}
              className="w-full text-left px-3 py-1.5 text-xs sm:text-sm text-white hover:bg-[#1a3157] first:rounded-t-lg last:rounded-b-lg"
            >
              Warden Testnet
            </button>
          </div>
