...
const NavBar: React.FC<NavBarProps> = ({ 
  ...
  const NetworkSelector = () => {
    ...
    const networks = [
      ...
      { id: 'arbitrum_mainnet', label: 'Arbitrum' },
      { id: 'arbitrum_sepolia', label: 'Arbitrum Sepolia' },
      { id: 'ethereum_mainnet', label: 'Ethereum' },
      { id: 'ethereum_sepolia', label: 'Ethereum Sepolia' },
      { id: 'polygon_mainnet', label: 'Polygon' },
      { id: 'polygon_mumbai', label: 'Polygon Mumbai' },
      { id: 'optimism_mainnet', label: 'Optimism' },
      { id: 'optimism_sepolia', label: 'Optimism Sepolia' }
    ];
  
    const getCurrentNetworkLabel = () => {
      if (networkType === 'incorrect') return 'Incorrect Network';
      const network = networks.find(n => n.id === networkType);
      return network?.label || 'Select Network';
    };
    ...
    return (
      <div className="relative ml-auto">
        <button
          ...
          focus:outline-none focus:ring-1 transition-colors min-w-[140px]
          ${isCorrectNetwork 
        ...
        >
          <span className="mr-4 truncate">{getCurrentNetworkLabel()}</span>
          <svg
            className={`w-4 h-4 absolute right-2 ${isCorrectNetwork ? 'text-cyan-300' : 'text-red-400'}`}
            fill="none"
            stroke="currentColor"
            viewBox="0 0 24 24"
          >
        ...
        {isOpen && (
          <div className="absolute z-[100] right-0 mt-3 w-48 bg-[#112240] border border-white/20 rounded-lg shadow-lg">
            {networks.map((network) => (
              <button
                key={network.id}
                onClick={() => {
                  setNetworkType(network.id as NetworkType);
                  setIsOpen(false);
                }}
                className="w-full text-left px-3 py-1.5 text-xs sm:text-sm text-white hover:bg-[#1a3157] first:rounded-t-lg last:rounded-b-lg truncate"
              >
                {network.label}
              </button>
            ))}
